...
class ScriptSeed(scripts.ScriptBuiltinUI):
	...
	def ui(A,is_img2img):
		K='Seed resize from-1';J="')}";I="function(){setRandomSeed('";H='subseed';D='Variation seed'
		...
		with gr.Group(visible=_A,elem_id=A.elem_id('seed_extras'))as N:
			with gr.Row(elem_id=A.elem_id('subseed_row')):C=gr.Number(label=D,value=-1,elem_id=A.elem_id(H),precision=0);O=ToolButton(ui.random_symbol,elem_id=A.elem_id('random_subseed'));P=ToolButton(ui.reuse_symbol,elem_id=A.elem_id('reuse_subseed'));E=gr.Slider(label='Variation strength',value=.0,minimum=0,maximum=1,step=.01,elem_id=A.elem_id(_E))
			with gr.Row(elem_id=A.elem_id('seed_resize_from_row')):F=gr.Slider(minimum=0,maximum=2048,step=8,label='Resize seed from width',value=0,elem_id=A.elem_id('seed_resize_from_w'));G=gr.Slider(minimum=0,maximum=2048,step=8,label='Resize seed from height',value=0,elem_id=A.elem_id('seed_resize_from_h'))
		L.click(fn=_B,_js=I+A.elem_id(_C)+J,show_progress=_A,inputs=[],outputs=[]);O.click(fn=_B,_js=I+A.elem_id(H)+J,show_progress=_A,inputs=[],outputs=[]);B.change(lambda x:gr.update(visible=x),show_progress=_A,inputs=[B],outputs=[N]);A.infotext_fields=[(A.seed,_D),(B,lambda d:D in d or K in d),(C,D),(E,'Variation seed strength'),(F,K),(G,'Seed resize from-2')];A.on_after_component(lambda x:connect_reuse_seed(A.seed,M,x.component,_A),elem_id=f"generation_info_{A.tabname}");A.on_after_component(lambda x:connect_reuse_seed(C,P,x.component,True),elem_id=f"generation_info_{A.tabname}");return A.seed,B,C,E,F,G
	def setup(E,p,seed,seed_checkbox,subseed,subseed_strength,seed_resize_from_w,seed_resize_from_h):
		D=seed_resize_from_h;C=seed_resize_from_w;B=subseed_strength;A=seed_checkbox;p.seed=seed
		if A and B>0:p.subseed=subseed;p.subseed_strength=B
		if A and C>0 and D>0:p.seed_resize_from_w=C;p.seed_resize_from_h=D
