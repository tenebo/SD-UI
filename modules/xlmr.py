...
class BertSeriesModelWithTransformation(BertPreTrainedModel):
	...
	def encode(B,c):E='attention_mask';D='input_ids';C=next(B.parameters()).device;A=B.tokenizer(c,truncation=_B,max_length=77,return_length=_C,return_overflowing_tokens=_C,padding='max_length',return_tensors='pt');A[D]=torch.tensor(A[D]).to(C);A[E]=torch.tensor(A[E]).to(C);F=B(**A);return F[_E]
