...
class Api:
	def __init__(self,app,queue_lock):
		...
		if shared.cmd_opts.api_server_stop:self.add_api_route('/sdapi/v1/server-kill',self.kill_webui,methods=[A]);self.add_api_route('/sdapi/v1/server-restart',self.restart_webui,methods=[A]);self.add_api_route('/sdapi/v1/server-stop',self.stop_webui,methods=[A])
	...
	def kill_webui(self):restart.stop_program()
	def restart_webui(self):
		if restart.is_restartable():restart.restart_program()
		return Response(status_code=501)
	def stop_webui(request):shared.state.server_command='stop';return Response('Stopping.')