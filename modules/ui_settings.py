...
class UiSettings:
	...
	def create_ui(A,loadsave,dummy_component):
		M='licenses';L='download_localization';K='primary';F=dummy_component;A.components=[];A.component_dict={};A.dummy_component=F;shared.settings_components=A.component_dict;script_callbacks.ui_settings_callback();opts.reorder()
		with gr.Blocks(analytics_enabled=_B)as N:
			with gr.Row():
				with gr.Column(scale=6):A.submit=gr.Button(value='Apply settings',variant=K,elem_id='settings_submit')
				with gr.Column():O=gr.Button(value='Reload UI',variant=K,elem_id='settings_restart_gradio')
			A.result=gr.HTML(elem_id='settings_result');A.quicksettings_names=opts.quicksettings_list;A.quicksettings_names={A:B for(B,A)in enumerate(A.quicksettings_names)if A!=_C};A.quicksettings_list=[];G=_A;B=_A;D=_A
			with gr.Tabs(elem_id='settings'):
				for(P,(E,C))in enumerate(opts.data_labels.items()):
					H=C.section[0]is _A
					if G!=C.section and not H:
						Q,R=C.section
						if B is not _A:D.__exit__();B.__exit__()
						gr.Group();B=gr.TabItem(elem_id=f"settings_{Q}",label=R);B.__enter__();D=gr.Column(variant=_D);D.__enter__();G=C.section
					...
					elif H:A.components.append(F)
					else:I=create_setting_component(E);A.component_dict[E]=I;A.components.append(I)
				if B is not _A:D.__exit__();B.__exit__()
				...
				with gr.TabItem('Sysinfo',id='sysinfo',elem_id='settings_tab_sysinfo'):
					...
					with gr.Row():
						with gr.Column(scale=1):J=gr.File(label='Check system info for validity',type='binary')
						with gr.Column(scale=1):S=gr.HTML('',elem_id='sysinfo_validity')
						with gr.Column(scale=100):0
				with gr.TabItem('Actions',id='actions',elem_id='settings_tab_actions'):
					T=gr.Button(value='Request browser notifications',elem_id='request_notifications');U=gr.Button(value='Download localization template',elem_id=L);V=gr.Button(value='Reload custom script bodies (No ui updates, No restart)',variant='secondary',elem_id='settings_reload_script_bodies')
				...
				with gr.TabItem('Licenses',id=M,elem_id='settings_tab_licenses'):gr.HTML(shared.html('licenses.html'),elem_id=M)
			...
			W.click(fn=sd_models.unload_model_weights,inputs=[],outputs=[]);X.click(fn=sd_models.reload_model_weights,inputs=[],outputs=[]);T.click(fn=lambda:_A,inputs=[],outputs=[],_js='function(){}');U.click(fn=lambda:_A,inputs=[],outputs=[],_js=L)
			...
			def Z(x):
				...
				return'Invalid'
			J.change(fn=Z,inputs=[J],outputs=[S])
		A.interface=N
	...
	def add_functionality(A,demo):
		E='sd_model_checkpoint';A.submit.click(fn=wrap_gradio_call(lambda*B:A.run_settings(*B),extra_outputs=[gr.update()]),inputs=A.components,outputs=[A.text_settings,A.result])
		for(K,C,L)in A.quicksettings_list:
			B=A.component_dict[C];G=opts.data_labels[C]
			if isinstance(B,gr.Textbox):D=[B.submit,B.blur]
			elif hasattr(B,'release'):D=[B.release]
			else:D=[B.change]
			for H in D:H(fn=lambda value,k=C:A.run_settings_single(value,key=k),inputs=[B],outputs=[B,A.text_settings],show_progress=G.refresh is not _A)
		I=gr.Button('Change checkpoint',elem_id='change_checkpoint',visible=_B);I.click(fn=lambda value,_:A.run_settings_single(value,key=E),_js="function(v){ var res = desiredCheckpointName; desiredCheckpointName = ''; return [res || v, null]; }",inputs=[A.component_dict[E],A.dummy_component],outputs=[A.component_dict[E],A.text_settings]);F=[B for B in opts.data_labels.keys()if B in A.component_dict]
