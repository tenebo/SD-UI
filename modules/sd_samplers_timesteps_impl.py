...
def unipc(model,x,timesteps,extra_args=_A,callback=_A,disable=_A,is_img2img=False):A=timesteps;B=model;C=B.inner_model.inner_model.alphas_cumprod;D=uni_pc.NoiseScheduleVP('discrete',alphas_cumprod=C);E=A[-1]/1000+1/1000 if is_img2img else _A;F=UniPCCFG(B,extra_args,callback,D,predict_x0=True,thresholding=False,variant=shared.opts.uni_pc_variant);x=F.sample(x,steps=len(A),t_start=E,skip_type=shared.opts.uni_pc_skip_type,method='multistep',order=shared.opts.uni_pc_order,lower_order_final=shared.opts.uni_pc_lower_order_final);return x