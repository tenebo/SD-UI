...
def cumsum_fix(input,cumsum_func,*D,**A):
	C=cumsum_func
	if input.device.type==_A:
		B=A.get('dtype',input.dtype)
		if B==torch.int64:return C(input.cpu(),*D,**A).to(input.device)
		elif B==torch.bool or cumsum_needs_int_fix and(B==torch.int8 or B==torch.int16):return C(input.to(torch.int32),*D,**A).to(torch.int64)
	return C(input,*D,**A)
if has_mps:
