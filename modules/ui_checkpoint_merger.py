...
class UiCheckpointMerger:
	def __init__(A):
		E='A, B or C';D='compact';C='None';B='choices'
		with gr.Blocks(analytics_enabled=_A)as F:
			with gr.Row(equal_height=_A):
				with gr.Column(variant=D):
					...
					with FormRow():
						with gr.Column():A.config_source=gr.Radio(choices=[E,'B','C',"Don't"],value=E,label='Copy config from',type='index',elem_id='modelmerger_config_method')
						with gr.Column():
				...
				with gr.Column(variant=D,elem_id='modelmerger_results_container'):
