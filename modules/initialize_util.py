...
def configure_opts_onchange():C=False;from modules import shared as A,sd_models as E,sd_vae as D,ui_tempdir as F,sd_hijack as G;from modules.call_queue import wrap_queued_call as B;A.opts.onchange('sd_model_checkpoint',B(lambda:E.reload_model_weights()),call=C);A.opts.onchange('sd_vae',B(lambda:D.reload_vae_weights()),call=C);A.opts.onchange('sd_vae_overrides_per_model_preferences',B(lambda:D.reload_vae_weights()),call=C);A.opts.onchange('temp_dir',F.on_tmpdir_changed);A.opts.onchange('gradio_theme',A.reload_gradio_theme);A.opts.onchange('cross_attention_optimization',B(lambda:G.model_hijack.redo_hijack(A.sd_model)),call=C);startup_timer.record('opts onchange')
