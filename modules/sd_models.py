...
def list_models():
	...
	else:C='https://huggingface.co/runwayml/stable-diffusion-v1-5/resolve/main/v1-5-pruned-emaonly.safetensors'
	...
	def get_sd_model(A):
		...
		if A.sd_model is _A:
			with A.lock:
				...
				try:load_model()
				except Exception as B:errors.display(B,'loading stable diffusion model',full_traceback=_C);print('',file=sys.stderr);print('Stable diffusion model failed to load',file=sys.stderr);A.sd_model=_A
		return A.sd_model
...
def get_empty_cond(sd_model):
	A=sd_model;B=processing.StableDiffusionProcessingTxt2Img();extra_networks.activate(B,{})
