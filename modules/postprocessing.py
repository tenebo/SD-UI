...
def run_postprocessing(extras_mode,image,image_folder,input_dir,output_dir,show_extras_results,*O,save_output=_A):
	N='extras';I=output_dir;F=extras_mode;C='';B=None;devices.torch_gc();shared.state.begin(job=N);J=[]
	def P(extras_mode,image,image_folder,input_dir):
		F=input_dir;E=extras_mode;A=image
		if E==1:
			...
			assert not shared.cmd_opts.hide_ui_dir_config,'--hide-ui-dir-config option must be disabled';assert F,'input directory not selected';I=shared.listfiles(F)
			for H in I:
				try:A=Image.open(H)
	...
	if F==2 and I!=C:K=I
	else:K=opts.outdir_samples or opts.outdir_extras_samples
	D=C
	for(E,G)in P(F,image,image_folder,input_dir):
		E:0;shared.state.textinfo=G;L,H=images.read_info_from_image(E)
		if L:H['parameters']=L
		A=scripts_postprocessing.PostprocessedImage(E.convert('RGB'));scripts.scripts_postproc.run(A,O)
		if opts.use_original_name_batch and G is not B:M=os.path.splitext(os.path.basename(G))[0]
		else:M=C
		...
		if save_output:images.save_image(A.image,path=K,basename=M,seed=B,prompt=B,extension=opts.samples_format,info=D,short_filename=_A,no_prompt=_A,grid=False,pnginfo_section_name=N,existing_info=H,forced_filename=B)
		if F!=2 or show_extras_results:J.append(A.image)
		E.close()
	devices.torch_gc();return J,ui_common.plaintext_to_html(D),C
