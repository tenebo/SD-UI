...
class ReplaceHelper:
	...
	def replace(D,obj,field,func):
		B=field;A=obj;C=getattr(A,B,_A)
		if C is _A:return
		D.replaced.append((A,B,C));setattr(A,B,func);return C
	def restore(A):
...
class DisableInitialization(ReplaceHelper):
	...
	def __enter__(A):
		...
		def D(original,url,*C,**E):
			...
			try:
				F=D(A,*C,local_files_only=True,**E)
				if F is _A:F=D(A,*C,local_files_only=B,**E)
				return F
			except Exception:return D(A,*C,local_files_only=B,**E)
...
class InitializeOnMeta(ReplaceHelper):
	...
	def __enter__(A):
		...
		def B(x):x['device']='meta';return x
		D=A.replace(torch.nn.Linear,C,lambda*A,**C:D(*A,**B(C)));E=A.replace(torch.nn.Conv2d,C,lambda*A,**C:E(*A,**B(C)));F=A.replace(torch.nn.MultiheadAttention,C,lambda*A,**C:F(*A,**B(C)));A.replace(torch.nn.Module,'to',lambda*A,**B:_A)
	def __exit__(A,exc_type,exc_val,exc_tb):A.restore()
class LoadStateDictOnMeta(ReplaceHelper):
	...
	def __enter__(A):
		C='_load_from_state_dict'
		if shared.cmd_opts.disable_model_loading_ram_optimization:return
		H=A.state_dict;K=A.device
		def B(original,module,state_dict,prefix,*L,**M):
			I=prefix;F=state_dict;E=module;J=[]
			for(G,D)in E._parameters.items():
				if D is _A:continue
				B=I+G;C=H.pop(B,_A)
				if C is not _A:F[B]=C.to(dtype=A.get_weight_dtype(B));J.append(B)
				if D.is_meta:N=C.dtype if C is not _A else D.dtype;E._parameters[G]=torch.nn.parameter.Parameter(torch.zeros_like(D,device=K,dtype=N),requires_grad=D.requires_grad)
			for G in E._buffers:
				B=I+G;C=H.pop(B,_A)
				if C is not _A:F[B]=C;J.append(B)
			original(E,F,I,*L,**M)
			for B in J:F.pop(B,_A)
		...
		E=A.replace(torch.nn.Module,'load_state_dict',lambda*A,**B:D(E,*A,**B));F=A.replace(torch.nn.Module,C,lambda*A,**C:B(F,*A,**C));G=A.replace(torch.nn.Linear,C,lambda*A,**C:B(G,*A,**C));I=A.replace(torch.nn.Conv2d,C,lambda*A,**C:B(I,*A,**C));J=A.replace(torch.nn.MultiheadAttention,C,lambda*A,**C:B(J,*A,**C));L=A.replace(torch.nn.LayerNorm,C,lambda*A,**C:B(L,*A,**C));M=A.replace(torch.nn.GroupNorm,C,lambda*A,**C:B(M,*A,**C))
	def __exit__(A,exc_type,exc_val,exc_tb):A.restore()