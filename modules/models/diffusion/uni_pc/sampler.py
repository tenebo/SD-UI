...
class UniPCSampler:
	...
	def sample(self,S,batch_size,shape,conditioning=_A,callback=_A,normals_sequence=_A,img_callback=_A,quantize_x0=False,eta=.0,mask=_A,x0=_A,temperature=1.,noise_dropout=.0,score_corrector=_A,corrector_kwargs=_A,verbose=True,x_T=_A,log_every_t=100,unconditional_guidance_scale=1.,unconditional_conditioning=_A,**Q):
		C=batch_size;B=self;A=conditioning
		if A is not _A:
			if isinstance(A,dict):
				D=A[list(A.keys())[0]]
				while isinstance(D,list):D=D[0]
				E=D.shape[0]
				if E!=C:print(f"Warning: Got {E} conditionings but batch-size is {C}")
			elif isinstance(A,list):
				for D in A:
					if D.shape[0]!=C:print(f"Warning: Got {E} conditionings but batch-size is {C}")
			elif A.shape[0]!=C:print(f"Warning: Got {A.shape[0]} conditionings but batch-size is {C}")
		I,J,K=shape;L=C,I,J,K;F=B.model.betas.device
		if x_T is _A:G=torch.randn(L,device=F)
		else:G=x_T
		H=NoiseScheduleVP('discrete',alphas_cumprod=B.alphas_cumprod);M='v'if B.model.parameterization=='v'else'noise';N=model_wrapper(lambda x,t,c:B.model.apply_model(x,t,c),H,model_type=M,guidance_type='classifier-free',guidance_scale=unconditional_guidance_scale);O=UniPC(N,H,predict_x0=True,thresholding=False,variant=shared.opts.uni_pc_variant,condition=A,unconditional_condition=unconditional_conditioning,before_sample=B.before_sample,after_sample=B.after_sample,after_update=B.after_update);P=O.sample(G,steps=S,skip_type=shared.opts.uni_pc_skip_type,method='multistep',order=shared.opts.uni_pc_order,lower_order_final=shared.opts.uni_pc_lower_order_final);return P.to(F),_A