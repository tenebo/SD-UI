...
class NoiseScheduleVP:
	def __init__(A,schedule=_D,betas=_A,alphas_cumprod=_A,continuous_beta_0=.1,continuous_beta_1=2e1):
		...
		A.schedule=B
		if B==_D:
			if E is not _A:C=.5*torch.log(1-E).cumsum(dim=0)
			else:assert D is not _A;C=.5*torch.log(D)
...
def model_wrapper(model,noise_schedule,model_type=_H,model_kwargs=_A,guidance_type=_I,guidance_scale=_B,classifier_fn=_A,classifier_kwargs=_A):
	...
	def I(x,t_continuous,cond=_A):
		...
		I=P(A)
		if cond is _A:E=O(x,I,_A,**H)
		else:E=O(x,I,cond,**H)
		if D==_H:return E
		elif D==M:G,F=C.marginal_alpha(A),C.marginal_std(A);B=x.dim();return(x-expand_dims(G,B)*E)/expand_dims(F,B)
		elif D=='v':G,F=C.marginal_alpha(A),C.marginal_std(A);B=x.dim();return expand_dims(G,B)*E+expand_dims(F,B)*x
		elif D=='score':F=C.marginal_std(A);B=x.dim();return-expand_dims(F,B)*E
	def R(x,t_input,condition):
		'\n        Compute the gradient of the classifier, i.e. nabla_{x} log p_t(cond | x_t).\n        '
		with torch.enable_grad():B=x.detach().requires_grad_(_C);C=N(B,t_input,condition,**A);return torch.autograd.grad(C.sum(),B)[0]
	def B(x,t_continuous,condition,unconditional_condition):
		...
		if G==_I:return I(x,B)
		elif G==L:assert N is not _A;S=P(B);M=R(x,S,A);T=C.marginal_std(B);H=I(x,B);return H-J*expand_dims(T,dims=M.dim())*M
		elif G==K:
			if J==_B or E is _A:return I(x,B,cond=A)
				...
				if isinstance(A,dict):
					...
					for O in range(len(A)):F.append(torch.cat([E[O],A[O]]))
				else:F=torch.cat([E,A])
				Q,H=I(U,V,cond=F).chunk(2);return Q+J*(H-Q)
	assert D in[_H,M,'v'];assert G in[_I,L,K];return B
class UniPC:
	...
	def get_time_steps(C,skip_type,t_T,t_0,N,device):
		'Compute the intermediate time steps for sampling.\n        ';D=t_0;E=t_T;B=skip_type;A=device
		if B==_J:G=C.noise_schedule.marginal_lambda(torch.tensor(E).to(A));H=C.noise_schedule.marginal_lambda(torch.tensor(D).to(A));I=torch.linspace(G.cpu().item(),H.cpu().item(),N+1).to(A);return C.noise_schedule.inverse_lambda(I)
		elif B==_K:return torch.linspace(E,D,N+1).to(A)
		elif B=='time_quadratic':F=2;J=torch.linspace(E**(_B/F),D**(_B/F),N+1).pow(F).to(A);return J
	...
	def get_orders_and_timesteps_for_singlestep_solver(G,steps,order,skip_type,t_T,t_0,device):
		'\n        Get the order of each step for sampling by the singlestep DPM-Solver.\n        ';D=device;E=skip_type;F=order;A=steps
		if F==3:
			B=A//3+1
		...
		if E==_J:H=G.get_time_steps(E,t_T,t_0,B,D)
		else:H=G.get_time_steps(E,t_T,t_0,A,D)[torch.cumsum(torch.tensor([0]+C),0).to(D)]
		return H,C
	def denoise_to_zero_fn(A,x,s):'\n        Denoise at the final step, which is equivalent to solve the ODE from lambda_s to infty by first-order discretization.\n        ';return A.data_prediction_fn(x,s)
	def multistep_uni_pc_update(A,x,model_prev_list,t_prev_list,t,order,**B):
		C=order;D=t_prev_list;E=model_prev_list
		if len(t.shape)==0:t=t.view(-1)
		if'bh'in A.variant:return A.multistep_uni_pc_bh_update(x,E,D,t,C,**B)
		else:assert A.variant=='vary_coeff';return A.multistep_uni_pc_vary_update(x,E,D,t,C,**B)
	def multistep_uni_pc_vary_update(H,x,model_prev_list,t_prev_list,t,order,use_corrector=_C):
		Z=order;a=t_prev_list;Q=use_corrector;R=model_prev_list;N='bkchw,k->bchw';D=H.noise_schedule;assert Z<=len(R);S=a[-1];b=D.marginal_lambda(S);h=D.marginal_lambda(t);I=R[-1];i,J=D.marginal_std(S),D.marginal_std(t);T=D.marginal_log_mean_coeff(t);O=torch.exp(T);U=h-b;F=[];C=[]
		for c in range(1,Z):j=a[-(c+1)];k=R[-(c+1)];l=D.marginal_lambda(j);d=(l-b)/U;F.append(d);C.append((k-I)/d)
		...
		if H.predict_x0:
			M=J/i*x-O*W*I;B=M
			if len(C)>0:
				for A in range(E-1):B=B-O*G[A+1]*torch.einsum(N,C,e[A])
			if Q:
				L=H.model_fn(B,t);Y=L-I;B=M;A=0
				for A in range(E-1):B=B-O*G[A+1]*torch.einsum(N,C,P[A][:-1])
				B=B-O*G[E]*(Y*P[A][-1])
		else:
			o,T=D.marginal_log_mean_coeff(S),D.marginal_log_mean_coeff(t);M=torch.exp(T-o)*x-J*W*I;B=M
			if len(C)>0:
				for A in range(E-1):B=B-J*G[A+1]*torch.einsum(N,C,e[A])
			if Q:
				L=H.model_fn(B,t);Y=L-I;B=M;A=0
				for A in range(E-1):B=B-J*G[A+1]*torch.einsum(N,C,P[A][:-1])
				B=B-J*G[E]*(Y*P[A][-1])
		return B,L
	def multistep_uni_pc_bh_update(D,x,model_prev_list,t_prev_list,t,order,x_t=_A,use_corrector=_C):
		e=t_prev_list;U=use_corrector;V=model_prev_list;S='k,bkchw->bchw';H=order;B=x_t;E=D.noise_schedule;assert H<=len(V);C=x.dim();W=e[-1];f=E.marginal_lambda(W);j=E.marginal_lambda(t);I=V[-1];k,T=E.marginal_std(W),E.marginal_std(t);l,g=E.marginal_log_mean_coeff(W),E.marginal_log_mean_coeff(t);X=torch.exp(g);Y=j-f;J=[];A=[]
		for K in range(1,H):m=e[-(K+1)];n=V[-(K+1)];o=E.marginal_lambda(m);h=((o-f)/Y)[0];J.append(h);A.append((n-I)/h)
		...
		O=_A
		if D.predict_x0:
			P=expand_dims(T/k,C)*x-expand_dims(X*Z,C)*I
			if B is _A:
				if i:Q=torch.einsum(S,c,A)
				else:Q=0
				B=P-expand_dims(X*G,C)*Q
			if U:
				O=D.model_fn(B,t)
				if A is not _A:R=torch.einsum(S,N[:-1],A)
				else:R=0
				d=O-I;B=P-expand_dims(X*G,C)*(R+N[-1]*d)
		else:
			P=expand_dims(torch.exp(g-l),C)*x-expand_dims(T*Z,C)*I
			if B is _A:
				if i:Q=torch.einsum(S,c,A)
				else:Q=0
				B=P-expand_dims(T*G,C)*Q
			if U:
				O=D.model_fn(B,t)
				if A is not _A:R=torch.einsum(S,N[:-1],A)
				else:R=0
				d=O-I;B=P-expand_dims(T*G,C)*(R+N[-1]*d)
		return B,O
	def sample(A,x,steps=20,t_start=_A,t_end=_A,order=3,skip_type=_K,method='singlestep',lower_order_final=_C,denoise_to_zero=_E,solver_type='dpm_solver',atol=.0078,rtol=.05,corrector=_E):
		K=t_end;L=t_start;E=order;D=steps;M=_B/A.noise_schedule.total_N if K is _A else K;S=A.noise_schedule.T if L is _A else L;N=x.device
		if method=='multistep':
