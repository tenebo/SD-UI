		...
		for _ in range((steps-initial_step)*gradient_step):
			...
			for(j,batch)in enumerate(dl):
				...
				if images_dir is not _A and steps_done%create_image_every==0:
					forced_filename=f"{embedding_name}-{steps_done}";last_saved_image=os.path.join(images_dir,forced_filename);shared.sd_model.first_stage_model.to(devices.device);p=processing.StandardDemoProcessingTxt2Img(sd_model=shared.sd_model,do_not_save_grid=_B,do_not_save_samples=_B,do_not_reload_embeddings=_B)
