...
def preprocess(id_task,process_src,process_dst,process_width,process_height,preprocess_txt_action,process_keep_original_size,process_flip,process_split,process_caption,process_caption_deepbooru=_B,split_threshold=.5,overlap_ratio=.2,process_focal_crop=_B,process_focal_crop_face_weight=.9,process_focal_crop_entropy_weight=.15,process_focal_crop_edges_weight=.5,process_focal_crop_debug=_B,process_multicrop=_A,process_multicrop_mindim=_A,process_multicrop_maxdim=_A,process_multicrop_minarea=_A,process_multicrop_maxarea=_A,process_multicrop_objective=_A,process_multicrop_threshold=_A):
	B=process_caption_deepbooru;A=process_caption
	try:
		if A:shared.interrogator.load()
		if B:deepbooru.model.start()
		preprocess_work(process_src,process_dst,process_width,process_height,preprocess_txt_action,process_keep_original_size,process_flip,process_split,A,B,split_threshold,overlap_ratio,process_focal_crop,process_focal_crop_face_weight,process_focal_crop_entropy_weight,process_focal_crop_edges_weight,process_focal_crop_debug,process_multicrop,process_multicrop_mindim,process_multicrop_maxdim,process_multicrop_minarea,process_multicrop_maxarea,process_multicrop_objective,process_multicrop_threshold)
	finally:
		if A:shared.interrogator.send_blip_to_ram()
		if B:deepbooru.model.stop()
...
def save_pic(image,index,params,existing_caption=_A):
	D=existing_caption;C=index;B=image;A=params;save_pic_with_caption(B,C,A,existing_caption=D)
	if A.flip:save_pic_with_caption(ImageOps.mirror(B),C,A,existing_caption=D)
def split_pic(image,inverse_xy,width,height,overlap_ratio):
	I=overlap_ratio;H=height;G=width;F=inverse_xy;A=image
	if F:J,K=A.height,A.width;B,C=H,G
	else:J,K=A.width,A.height;B,C=G,H
	D=K*B//J
	if F:A=A.resize((D,B))
	else:A=A.resize((B,D))
	L=math.ceil((D-C*I)/(C*(1.-I)));N=(D-C)/(L-1)
	for O in range(L):
		E=int(N*O)
...
def multicrop_pic(image,mindim,maxdim,minarea,maxarea,objective,threshold):D=maxdim;C=mindim;A=image;F,G=A.size;E=lambda w,h:1-(lambda x:x if x<1 else 1/x)(F/G/(w/h));B=max(((A,B)for A in range(C,D+1,64)for B in range(C,D+1,64)if minarea<=A*B<=maxarea and E(A,B)<=threshold),key=lambda wh:(wh[0]*wh[1],-E(*wh))[::1 if objective=='Maximize area'else-1],default=_A);return B and center_crop(A,*B)
def preprocess_work(process_src,process_dst,process_width,process_height,preprocess_txt_action,process_keep_original_size,process_flip,process_split,process_caption,process_caption_deepbooru=_B,split_threshold=.5,overlap_ratio=.2,process_focal_crop=_B,process_focal_crop_face_weight=.9,process_focal_crop_entropy_weight=.3,process_focal_crop_edges_weight=.5,process_focal_crop_debug=_B,process_multicrop=_A,process_multicrop_mindim=_A,process_multicrop_maxdim=_A,process_multicrop_minarea=_A,process_multicrop_maxarea=_A,process_multicrop_objective=_A,process_multicrop_threshold=_A):
	O=True;J=overlap_ratio;I=split_threshold;E=process_width;F=process_height;K=os.path.abspath(process_src);L=os.path.abspath(process_dst);I=max(.0,min(1.,I));J=max(.0,min(.9,J));assert K!=L,'same directory specified as source and destination';os.makedirs(L,exist_ok=O);M=listfiles(K);shared.state.job='preprocess';shared.state.textinfo='Preprocessing...';shared.state.job_count=len(M);B=PreprocessParams();B.dstdir=L;B.flip=process_flip;B.process_caption=process_caption;B.process_caption_deepbooru=process_caption_deepbooru;B.preprocess_txt_action=preprocess_txt_action;P=tqdm.tqdm(M)
	for(C,V)in enumerate(P):
		B.subindex=0;H=os.path.join(K,V)
		try:A=Image.open(H);A=ImageOps.exif_transpose(A);A=A.convert('RGB')
		except Exception:continue
		Q=f"Preprocessing [Image {C}/{len(M)}]";P.set_description(Q);shared.state.textinfo=Q;B.src=H;D=_A;R=f"{os.path.splitext(H)[0]}.txt"
		if os.path.exists(R):
			with open(R,'r',encoding='utf8')as W:D=W.read()
		if shared.state.interrupted:break
		if A.height>A.width:N=A.width*F/(A.height*E);S=_B
		else:N=A.height*E/(A.width*F);S=O
		G=O
		if process_split and N<1. and N<=I:
