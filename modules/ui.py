...
def create_ui():
	...
	with gr.Blocks(theme=shared.gradio_theme,analytics_enabled=_A,title='Standard Demo')as o:
		...
		AM=shared.html('footer.html');AM=AM.format(versions=versions_html(),api_docs='/docs'if shared.cmd_opts.api else'https://github.com/tenebo/standard-demo-we/wiki/API');gr.HTML(AM,elem_id='footer');S.add_functionality(o);BB=lambda:gr.update(visible=shared.sd_model and shared.sd_model.cond_stage_key=='edit');S.text_settings.change(fn=BB,inputs=[],outputs=[m]);o.load(fn=BB,inputs=[],outputs=[m]);At.setup_ui(dummy_component=C,sd_model_checkpoint_component=S.component_dict[_O])
