...
var titles = {
  ...
  "Euler a":
    "Euler Ancestral - very creative, each can get a completely different picture depending on step count, setting steps higher than 30-40 does not help",
  ...
  "\u267b\ufe0f":
    "Reuse seed from last generation, mostly useful if it was randomized",
  "\u2199\ufe0f":
  ...
  "Masked content":
    "What to put inside the masked area before processing it with Standard Demo.",
  ...
  "X/Y/Z plot":
    "Create grid(s) where images will have different parameters. Use inputs below to specify which parameters will be shared by columns and rows",
  "Custom code":
  ...
  "Images filename pattern":
    "Use tags like [seed] and [date] to define how filenames for images are chosen. Leave empty for default.",
  "Directory name pattern":
    "Use tags like [seed] and [date] to define how subdirectories for images and grids are chosen. Leave empty for default.",
  "Max prompt words":
  ...
  Loopback:
    "Performs img2img processing multiple times. Output images are used as input for the next loop.",
  Loops:
  ...
  "Final denoising strength":
    "The denoising strength for the final loop of each image in the batch.",
  "Denoising strength curve":
    "The denoising curve controls the rate of denoising strength change each loop. Aggressive: Most of the change will happen towards the start of the loops. Linear: Change will be constant through all loops. Lazy: Most of the change will happen towards the end of the loops.",
  ...
  "Create style":
    "Save current prompts as a style. If you add the token {prompt} to the text, the style uses that as a placeholder for your prompt when you use the style in the future.",
  ...
  "Quicksettings list":
    "List of setting names, separated by commas, for settings that should go to the quick access bar at the top, rather than the usual setting tab. See modules/shared.py for setting names. Requires restarting to apply.",
  ...
  "Learning rate":
    "How fast should training go. Low values will take longer to train, high values may fail to converge (not generate accurate results) and/or may break the embedding (This has happened if you see Loss: nan in the training info textbox. If this happens, you need to manually restore your embedding from an older not-broken backup).\n\nYou can set a single numeric value, or multiple learning rates using the syntax:\n\n   rate_1:max_steps_1, rate_2:max_steps_2, ...\n\nEG:   0.005:100, 1e-3:1000, 1e-5\n\nWill train with rate of 0.005 for first 100 steps, then 1e-3 until 1000 steps, then 1e-5 for all remaining steps.",
  ...
  "Hires steps":
    "Number of sampling steps for upscaled picture. If 0, uses same as for original.",
  "Upscale by":
    "Adjusts the size of the image by multiplying the original width and height by the selected value. Ignored if either Resize width to or Resize height to are non-zero.",
  "Resize width to":
  ...
  "Discard weights with matching name":
    "Regular expression; if weights's name matches it, the weights is not written to the resulting checkpoint. Use ^model_ema to discard EMA weights.",
  "Extra networks tab order":
    "Comma-separated list of tab names; tabs listed here will appear in the extra networks UI first and in order listed.",
...
onUiUpdate(function (mutationRecords) {
  for (const record of mutationRecords) {
    if (
      ...
      record.target.classList.contains("options")
    ) {
...
onUiLoaded(function () {
  for (var comp of window.gradio_config.components) {
    if (comp.props.ourui_tooltip && comp.props.elem_id) {
      var elem = gradioApp().getElementById(comp.props.elem_id);
      if (elem) {
        elem.title = comp.props.ourui_tooltip;
      }
    }
  }
});
