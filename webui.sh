...
# Name of the subdirectory (defaults to standard-demo-we)
if [[ -z "${clone_dir}" ]]
then
    clone_dir="standard-demo-we"
fi
...
then
    ...
    printf "\n%s\n" "${delimiter}"
    printf "Clone standard-demo-we"
    printf "\n%s\n" "${delimiter}"
    "${GIT}" clone https://github.com/tenebo/standard-demo-we.git "${clone_dir}"
    cd "${clone_dir}"/ || { printf "\e[1m\e[31mERROR: Can't cd to %s/%s/, aborting...\e[0m" "${install_dir}" "${clone_dir}"; exit 1; }
fi
