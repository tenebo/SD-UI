...
class LDSR:
	def load_model_from_config(B,half_attention):
		F='state_dict';E='cpu';global cached_ldsr_model
		if shared.opts.ldsr_cached and cached_ldsr_model is not _A:print('Loading model from cache');A=cached_ldsr_model
			...
			H=C[F]if F in C else C;D=OmegaConf.load(B.yamlPath);D.model.target='ldm.models.diffusion.ddpm.LatentDiffusionV1';A=instantiate_from_config(D.model);A.load_state_dict(H,strict=_C);A=A.to(shared.device)
...
def get_cond(selected_path):E='1 c h w -> 1 h w c';B={};D=4;A=selected_path.convert('RGB');A=torch.unsqueeze(torchvision.transforms.ToTensor()(A),0);C=torchvision.transforms.functional.resize(A,size=[D*A.shape[2],D*A.shape[3]],antialias=_D);C=rearrange(C,E);A=rearrange(A,E);A=2.*A-_B;A=A.to(shared.device);B['LR_image']=A;B['image']=C;return B
@torch.no_grad()
...
def make_convolutional_sample(batch,model,custom_steps=_A,eta=_B,quantize_x0=_C,custom_shape=_A,temperature=_B,noise_dropout=.0,corrector=_A,corrector_kwargs=_A,x_T=_A,ddim_use_x0_pred=_C):
	J='original_conditioning';D=custom_shape;A=model;B={};G,K,E,L,C=A.get_input(batch,A.first_stage_key,return_first_stage_outputs=_D,force_c_encode=not(hasattr(A,_E)and A.cond_stage_key=='coordinates_bbox'),return_original_cond=_D)
	if D is not _A:G=torch.randn(D);print(f"Generating {D[0]} samples of shape {D[1:]}")
	M=_A;B['input']=E;B['reconstruction']=L
	if ismap(C):
		B[J]=A.to_rgb(C)
		if hasattr(A,'cond_stage_key'):B[A.cond_stage_key]=A.to_rgb(C)
	else:
		B[J]=C if C is not _A else torch.zeros_like(E)
	...
	with A.ema_scope('Plotting'):
		N=time.time();F,O=convsample_ddim(A,K,steps=custom_steps,shape=G.shape,eta=eta,quantize_x0=quantize_x0,mask=_A,x0=M,temperature=temperature,score_corrector=corrector,corrector_kwargs=corrector_kwargs,x_t=x_T);P=time.time()
		if ddim_use_x0_pred:F=O['pred_x0'][-1]
	H=A.decode_first_stage(F)
	try:I=A.decode_first_stage(F,force_not_quantize=_D);B['sample_noquant']=I;B['sample_diff']=torch.abs(I-H)
	except Exception:pass
	B[_F]=H;B['time']=P-N;return B