...
shared.options_templates.update(shared.options_section(('extra_networks','Extra Networks'),{'sd_lora':shared.OptionInfo('None','Add network to prompt',gr.Dropdown,lambda:{_A:['None',*networks.available_networks]},refresh=networks.list_available_networks),'lora_preferred_name':shared.OptionInfo(_C,'When adding to prompt, refer to Lora by',gr.Radio,{_A:[_C,'Filename']}),'lora_add_hashes_to_infotext':shared.OptionInfo(True,'Add Lora hashes to infotext'),'lora_show_all':shared.OptionInfo(False,'Always show all networks on the Lora page').info('otherwise, those detected as for incompatible version of Standard Demo will be hidden'),'lora_hide_unknown_for_versions':shared.OptionInfo([],'Hide networks of unknown versions for model versions',gr.CheckboxGroup,{_A:['SD1','SD2','SDXL']}),_B:shared.OptionInfo(0,'Number of Lora networks to keep cached in memory',gr.Number,{'precision':0})}))
shared.options_templates.update(shared.options_section(('compatibility','Compatibility'),{'lora_functional':shared.OptionInfo(False,'Lora/Networks: use old method that takes longer when you have multiple Loras active and produces same results as kohya-ss/sd-we-additional-networks extension')}))
