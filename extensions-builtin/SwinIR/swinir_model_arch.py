...
class Mlp(nn.Module):
	def __init__(A,in_features,hidden_features=_A,out_features=_A,act_layer=nn.GELU,drop=_B):D=out_features;C=in_features;B=hidden_features;super().__init__();D=D or C;B=B or C;A.fc1=nn.Linear(C,B);A.act=act_layer();A.fc2=nn.Linear(B,D);A.drop=nn.Dropout(drop)
...
class WindowAttention(nn.Module):
	...
	def __init__(A,dim,window_size,num_heads,qkv_bias=_C,qk_scale=_A,attn_drop=_B,proj_drop=_B):E=num_heads;D=window_size;B=dim;super().__init__();A.dim=B;A.window_size=D;A.num_heads=E;G=B//E;A.scale=qk_scale or G**-.5;A.relative_position_bias_table=nn.Parameter(torch.zeros((2*D[0]-1)*(2*D[1]-1),E));H=torch.arange(A.window_size[0]);I=torch.arange(A.window_size[1]);J=torch.stack(torch.meshgrid([H,I]));F=torch.flatten(J,1);C=F[:,:,_A]-F[:,_A,:];C=C.permute(1,2,0).contiguous();C[:,:,0]+=A.window_size[0]-1;C[:,:,1]+=A.window_size[1]-1;C[:,:,0]*=2*A.window_size[1]-1;K=C.sum(-1);A.register_buffer('relative_position_index',K);A.qkv=nn.Linear(B,B*3,bias=qkv_bias);A.attn_drop=nn.Dropout(attn_drop);A.proj=nn.Linear(B,B);A.proj_drop=nn.Dropout(proj_drop);trunc_normal_(A.relative_position_bias_table,std=.02);A.softmax=nn.Softmax(dim=-1)
...
class SwinTransformerBlock(nn.Module):
	...
	def __init__(A,dim,input_resolution,num_heads,window_size=7,shift_size=0,mlp_ratio=4.,qkv_bias=_C,qk_scale=_A,drop=_B,attn_drop=_B,drop_path=_B,act_layer=nn.GELU,norm_layer=nn.LayerNorm):
		...
		assert 0<=A.shift_size<A.window_size,'shift_size must in 0-window_size';A.norm1=F(B);A.attn=WindowAttention(B,window_size=to_2tuple(A.window_size),num_heads=C,qkv_bias=qkv_bias,qk_scale=qk_scale,attn_drop=attn_drop,proj_drop=drop);A.drop_path=DropPath(E)if E>_B else nn.Identity();A.norm2=F(B);H=int(B*D);A.mlp=Mlp(in_features=B,hidden_features=H,act_layer=act_layer,drop=drop)
...
class BasicLayer(nn.Module):
	...
	def __init__(A,dim,input_resolution,depth,num_heads,window_size,mlp_ratio=4.,qkv_bias=_C,qk_scale=_A,drop=_B,attn_drop=_B,drop_path=_B,norm_layer=nn.LayerNorm,downsample=_A,use_checkpoint=_D):
		H=downsample;G=norm_layer;F=window_size;E=depth;D=drop_path;C=input_resolution;B=dim;super().__init__();A.dim=B;A.input_resolution=C;A.depth=E;A.use_checkpoint=use_checkpoint;A.blocks=nn.ModuleList([SwinTransformerBlock(dim=B,input_resolution=C,num_heads=num_heads,window_size=F,shift_size=0 if A%2==0 else F//2,mlp_ratio=mlp_ratio,qkv_bias=qkv_bias,qk_scale=qk_scale,drop=drop,attn_drop=attn_drop,drop_path=D[A]if isinstance(D,list)else D,norm_layer=G)for A in range(E)])
...
class RSTB(nn.Module):
	...
	def __init__(B,dim,input_resolution,depth,num_heads,window_size,mlp_ratio=4.,qkv_bias=_C,qk_scale=_A,drop=_B,attn_drop=_B,drop_path=_B,norm_layer=nn.LayerNorm,downsample=_A,use_checkpoint=_D,img_size=224,patch_size=4,resi_connection=_E):
		...
		elif F=='3conv':B.conv=nn.Sequential(nn.Conv2d(A,A//4,3,1,1),nn.LeakyReLU(negative_slope=.2,inplace=_C),nn.Conv2d(A//4,A//4,1,1,0),nn.LeakyReLU(negative_slope=.2,inplace=_C),nn.Conv2d(A//4,A,3,1,1))
		B.patch_embed=PatchEmbed(img_size=D,patch_size=E,in_chans=0,embed_dim=A,norm_layer=_A);B.patch_unembed=PatchUnEmbed(img_size=D,patch_size=E,in_chans=0,embed_dim=A,norm_layer=_A)
...
class PatchEmbed(nn.Module):
	...
	def __init__(A,img_size=224,patch_size=4,in_chans=3,embed_dim=96,norm_layer=_A):
		F=norm_layer;E=embed_dim;C=patch_size;B=img_size;super().__init__();B=to_2tuple(B);C=to_2tuple(C);D=[B[0]//C[0],B[1]//C[1]];A.img_size=B;A.patch_size=C;A.patches_resolution=D;A.num_patches=D[0]*D[1];A.in_chans=in_chans;A.embed_dim=E
		if F is not _A:A.norm=F(E)
		else:A.norm=_A
	def forward(A,x):
...
class SwinIR(nn.Module):
	...
	def __init__(A,img_size=64,patch_size=1,in_chans=3,embed_dim=96,depths=(6,6,6,6),num_heads=(6,6,6,6),window_size=7,mlp_ratio=4.,qkv_bias=_C,qk_scale=_A,drop_rate=_B,attn_drop_rate=_B,drop_path_rate=.1,norm_layer=nn.LayerNorm,ape=_D,patch_norm=_C,use_checkpoint=_D,upscale=2,img_range=1.,upsampler='',resi_connection=_E,**U):
		O=drop_rate;N=window_size;M=resi_connection;L=upscale;K=in_chans;J=patch_size;I=img_size;F=norm_layer;D=depths;B=embed_dim;super(SwinIR,A).__init__();P=K;G=K;C=64;A.img_range=img_range
		if K==3:Q=.4488,.4371,.404;A.mean=torch.Tensor(Q).view(1,3,1,1)
		else:A.mean=torch.zeros(1,1,1,1)
		A.upscale=L;A.upsampler=upsampler;A.window_size=N;A.conv_first=nn.Conv2d(P,B,3,1,1);A.num_layers=len(D);A.embed_dim=B;A.ape=ape;A.patch_norm=patch_norm;A.num_features=B;A.mlp_ratio=mlp_ratio;A.patch_embed=PatchEmbed(img_size=I,patch_size=J,in_chans=B,embed_dim=B,norm_layer=F if A.patch_norm else _A);R=A.patch_embed.num_patches;E=A.patch_embed.patches_resolution;A.patches_resolution=E;A.patch_unembed=PatchUnEmbed(img_size=I,patch_size=J,in_chans=B,embed_dim=B,norm_layer=F if A.patch_norm else _A)
		if A.ape:A.absolute_pos_embed=nn.Parameter(torch.zeros(1,R,B));trunc_normal_(A.absolute_pos_embed,std=.02)
		A.pos_drop=nn.Dropout(p=O);S=[A.item()for A in torch.linspace(0,drop_path_rate,sum(D))];A.layers=nn.ModuleList()
		for H in range(A.num_layers):T=RSTB(dim=B,input_resolution=(E[0],E[1]),depth=D[H],num_heads=num_heads[H],window_size=N,mlp_ratio=A.mlp_ratio,qkv_bias=qkv_bias,qk_scale=qk_scale,drop=O,attn_drop=attn_drop_rate,drop_path=S[sum(D[:H]):sum(D[:H+1])],norm_layer=F,downsample=_A,use_checkpoint=use_checkpoint,img_size=I,patch_size=J,resi_connection=M);A.layers.append(T)
		A.norm=F(A.num_features)
		if M==_E:A.conv_after_body=nn.Conv2d(B,B,3,1,1)
		elif M=='3conv':A.conv_after_body=nn.Sequential(nn.Conv2d(B,B//4,3,1,1),nn.LeakyReLU(negative_slope=.2,inplace=_C),nn.Conv2d(B//4,B//4,1,1,0),nn.LeakyReLU(negative_slope=.2,inplace=_C),nn.Conv2d(B//4,B,3,1,1))
		if A.upsampler==_G:A.conv_before_upsample=nn.Sequential(nn.Conv2d(B,C,3,1,1),nn.LeakyReLU(inplace=_C));A.upsample=Upsample(L,C);A.conv_last=nn.Conv2d(C,G,3,1,1)
		elif A.upsampler==_F:A.upsample=UpsampleOneStep(L,B,G,(E[0],E[1]))
		elif A.upsampler==_H:
	...
	def forward(A,x):
		C='nearest';D,E=x.shape[2:];x=A.check_image_size(x);A.mean=A.mean.type_as(x);x=(x-A.mean)*A.img_range
		if A.upsampler==_G:x=A.conv_first(x);x=A.conv_after_body(A.forward_features(x))+x;x=A.conv_before_upsample(x);x=A.conv_last(A.upsample(x))
			...
			x=A.conv_first(x);x=A.conv_after_body(A.forward_features(x))+x;x=A.conv_before_upsample(x);x=A.lrelu(A.conv_up1(torch.nn.functional.interpolate(x,scale_factor=2,mode=C)))
			if A.upscale==4:x=A.lrelu(A.conv_up2(torch.nn.functional.interpolate(x,scale_factor=2,mode=C)))
			x=A.conv_last(A.lrelu(A.conv_hr(x)))
		else:B=A.conv_first(x);F=A.conv_after_body(A.forward_features(B))+B;x=x+A.conv_last(F)
