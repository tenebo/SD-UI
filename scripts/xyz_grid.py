...
def boolean_choice(reverse=_C):
	def A():B='True';A='False';return[A,B]if reverse else[B,A]
	return A
...
def draw_xyz_grid(p,xs,ys,zs,x_labels,y_labels,z_labels,cell,draw_legend,include_lone_images,include_sub_grids,first_axes_processed,second_axes_processed,margin_size):
	S=draw_legend;P=second_axes_processed;O=first_axes_processed;J=zs;C=ys;B=xs;V=[[images.GridAnnotation(A)]for A in x_labels];W=[[images.GridAnnotation(A)]for A in y_labels];X=[[images.GridAnnotation(A)]for A in z_labels];L=len(B)*len(C)*len(J);A=_A;state.job_count=L*p.n_iter
	def M(x,y,z,ix,iy,iz):
		nonlocal A
		def F(ix,iy,iz):return ix+iy*len(B)+iz*len(B)*len(C)
		...
		for(D,E)in enumerate(B):
			if P==_D:
				...
				for(H,I)in enumerate(J):
					for(F,G)in enumerate(C):M(E,G,I,D,F,H)
	elif O==_D:
		for(F,G)in enumerate(C):
			if P=='x':
				for(D,E)in enumerate(B):
				...
				for(H,I)in enumerate(J):
					for(D,E)in enumerate(B):M(E,G,I,D,F,H)
	elif O=='z':
		for(H,I)in enumerate(J):
			if P=='x':
				for(D,E)in enumerate(B):
	...
	T=len(J)
	for N in range(T):
		K=N*len(B)*len(C)+N;Y=K+len(B)*len(C);Q=images.image_grid(A.images[K:Y],rows=len(C))
...
class Script(scripts.Script):
	...
	def ui(A,is_img2img):
		Z='compact';Y='Z values';X='Y values';W='X values';S='index';A.current_axis_options=[A for A in axis_options if type(A)==AxisOption or A.is_img2img==is_img2img]
		with gr.Row():
			with gr.Column(scale=19):
				with gr.Row():I=gr.Dropdown(label='X type',choices=[A.label for A in A.current_axis_options],value=A.current_axis_options[1].label,type=S,elem_id=A.elem_id('x_type'));B=gr.Textbox(label=W,lines=1,elem_id=A.elem_id('x_values'));C=gr.Dropdown(label=W,visible=_C,multiselect=_B,interactive=_B);M=ToolButton(value=fill_values_symbol,elem_id='xyz_grid_fill_x_tool_button',visible=_C)
				with gr.Row():J=gr.Dropdown(label='Y type',choices=[A.label for A in A.current_axis_options],value=A.current_axis_options[0].label,type=S,elem_id=A.elem_id('y_type'));D=gr.Textbox(label=X,lines=1,elem_id=A.elem_id('y_values'));E=gr.Dropdown(label=X,visible=_C,multiselect=_B,interactive=_B);N=ToolButton(value=fill_values_symbol,elem_id='xyz_grid_fill_y_tool_button',visible=_C)
				with gr.Row():K=gr.Dropdown(label='Z type',choices=[A.label for A in A.current_axis_options],value=A.current_axis_options[0].label,type=S,elem_id=A.elem_id('z_type'));F=gr.Textbox(label=Y,lines=1,elem_id=A.elem_id('z_values'));G=gr.Dropdown(label=Y,visible=_C,multiselect=_B,interactive=_B);O=ToolButton(value=fill_values_symbol,elem_id='xyz_grid_fill_z_tool_button',visible=_C)
		with gr.Row(variant=Z,elem_id='axis_options'):
		...
		with gr.Row(variant=Z,elem_id='swap_axes'):f=gr.Button(value='Swap X/Y axes',elem_id='xy_grid_swap_axes_button');g=gr.Button(value='Swap Y/Z axes',elem_id='yz_grid_swap_axes_button');h=gr.Button(value='Swap X/Z axes',elem_id='xz_grid_swap_axes_button')
		def P(axis1_type,axis1_values,axis1_values_dropdown,axis2_type,axis2_values,axis2_values_dropdown):return A.current_axis_options[axis2_type].label,axis2_values,axis2_values_dropdown,A.current_axis_options[axis1_type].label,axis1_values,axis1_values_dropdown
		T=[I,B,C,J,D,E];f.click(P,inputs=T,outputs=T);U=[J,D,E,K,F,G];g.click(P,inputs=U,outputs=U);V=[I,B,C,K,F,G];h.click(P,inputs=V,outputs=V)
		def Q(axis_type,csv_mode):
		...
		M.click(fn=Q,inputs=[I,H],outputs=[B,C]);N.click(fn=Q,inputs=[J,H],outputs=[D,E]);O.click(fn=Q,inputs=[K,H],outputs=[F,G])
		def L(axis_type,axis_values,axis_values_dropdown,csv_mode):
			F=csv_mode;D=axis_values_dropdown;C=axis_values;B=A.current_axis_options[axis_type].choices;E=B is not _A
			if E:
				B=B()
				if F:
					if D:C=list_to_csv_string(list(filter(lambda x:x in B,D)));D=[]
				elif C:D=list(filter(lambda x:x in B,csv_string_to_list_strip(C)));C=''
			return gr.Button.update(visible=E),gr.Textbox.update(visible=not E or F,value=C),gr.update(choices=B if E else _A,visible=E and not F,value=D)
		I.change(fn=L,inputs=[I,B,C,H],outputs=[M,B,C]);J.change(fn=L,inputs=[J,D,E,H],outputs=[N,D,E]);K.change(fn=L,inputs=[K,F,G,H],outputs=[O,F,G])
		...
		H.change(fn=i,inputs=[H,I,B,C,J,D,E,K,F,G],outputs=[M,B,C,N,D,E,O,F,G])
		def R(axis,params):A=f"{axis} Values";B=params.get(A,'');C=csv_string_to_list_strip(B);return gr.update(value=C)
		A.infotext_fields=(I,_P),(B,_Q),(C,lambda params:R('X',params)),(J,_R),(D,_S),(E,lambda params:R('Y',params)),(K,_T),(F,_U),(G,lambda params:R('Z',params));return[I,B,C,J,D,E,K,F,G,a,c,d,b,e,H]
	def run(N,p,x_type,x_values,x_values_dropdown,y_type,y_values,y_values_dropdown,z_type,z_values,z_values_dropdown,draw_legend,include_lone_images,include_sub_grids,no_fixed_seeds,margin_size,csv_mode):
		a=include_sub_grids;Z=include_lone_images;Y=z_values_dropdown;X=y_values_dropdown;W=x_values_dropdown;S=z_values;R=y_values;Q=x_values;O=csv_mode;K=no_fixed_seeds
		...
		if C.choices is not _A and not O:Q=list_to_csv_string(W)
		D=T(C,Q,W);E=N.current_axis_options[y_type]
		if E.choices is not _A and not O:R=list_to_csv_string(X)
		F=T(E,R,X);G=N.current_axis_options[z_type]
		if G.choices is not _A and not O:S=list_to_csv_string(Y)
		B=T(G,S,Y);Image.MAX_IMAGE_PIXELS=_A;b=round(len(D)*len(F)*len(B)*p.width*p.height/1000000);assert b<opts.img_max_size_mp,f"Error: Resulting grid would be too large ({b} MPixels) (max configured size is {opts.img_max_size_mp} MPixels)"
		...
		def f(x,y,z,ix,iy,iz):
			...
			if L[I]is _A and ix==0 and iy==0:
				...
				if C.label!=_E:
					A.extra_generation_params[_P]=C.label;A.extra_generation_params[_Q]=Q
			...
			if L[0]is _A and ix==0 and iy==0 and iz==0:
				...
				if G.label!=_E:
					A.extra_generation_params[_T]=G.label;A.extra_generation_params[_U]=S
			...
			return H
		with SharedSettingsStackHelper():A=draw_xyz_grid(p,xs=D,ys=F,zs=B,x_labels=[C.format_value(p,C,A)for A in D],y_labels=[E.format_value(p,E,A)for A in F],z_labels=[G.format_value(p,G,A)for A in B],cell=f,draw_legend=draw_legend,include_lone_images=Z,include_sub_grids=a,first_axes_processed=P,second_axes_processed=I,margin_size=margin_size)
		if not A.images:return A
		J=len(B);A.infotexts[:1+J]=L[:1+J]
		if not Z:A.images=A.images[:J+1]
		if opts.grid_save:
			g=J+1 if J>1 else 1
